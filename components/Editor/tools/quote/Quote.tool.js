...
import { QuoteUp, QuoteDown } from './Quote.constants';
...
export default class Quote {
  ...
  get CSS() {
    return {
      ...
      wrapper: classes['cdx-quote'],
      body: classes.body,
      text: classes['cdx-quote__text'],
      input: this.api.styles.input,
      footer: classes.footer,
      prefix: classes.prefix,
      caption: classes['cdx-quote__caption'],
      quoteUp: classes['quote-up'],
      quoteDown: classes['quote-down'],
    };
  }
   ...
   */
  render() {
    const container = this._make('blockquote', [this.CSS.wrapper]);
    const body = this._make('div', this.CSS.body);
    container.appendChild(body);

    const quoteUp = this._make('div', this.CSS.quoteUp);
    quoteUp.innerHTML = QuoteUp;
    body.appendChild(quoteUp);

    this.quote = this._make('div', this.CSS.text, {
      contentEditable: !this.readOnly,
      innerHTML: this.data.text,
    });
    body.appendChild(this.quote);

    const quoteDown = this._make('div', this.CSS.quoteDown);
    quoteDown.innerHTML = QuoteDown;
    body.appendChild(quoteDown);

    const footer = this._make('div', this.CSS.footer);
    container.appendChild(footer);

    const prefix = this._make('span', this.CSS.prefix);
    prefix.innerHTML = '&mdash;';
